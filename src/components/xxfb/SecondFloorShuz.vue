<template>
  <div class="div" id="path">
    <div class="div3">
      <img src="http://39.105.179.38:9797/noderad/%E7%BE%A4%E7%BB%84%202%20Copy%205@2x.png" />
      <!-- 提示语 -->
      <div>
        <!-- <div>
          <img src="http://39.105.179.38:9797/noderad/200.png" />
          <span>温馨提示：</span>
        </div>
        <div>&nbsp;&nbsp;请说导航到xxx会议室<br />&nbsp;&nbsp;或说xxx会议室怎么走</div> -->
      </div>
    </div>
    <div class="div2cont" id="div2cont">
      <div class="div2" id="div2">
        <!-- 过道 -->
        <div class="div2-1"><br />过<br />道 <br /><br /><br />过<br />道</div>
        <!-- 中间中部 -->
        <div class="div2-2" id="div2-2">
          <div class="left">
            <div class="left1">过&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;道</div>
            <!-- 会议室1区 -->
            <div class="left2">
              <div class="left2-1">
                <div>
                  <div id="A2211" data-roomName="A2211" class="roomName">A2-211</div>
                  <!-- 竖双门 -->
                  <img
                    class="hdoubledoor"
                    src="http://39.105.179.38:9797/noderad/%E7%BE%A4%E7%BB%84%205%20Copy%2025@2x.png "
                  />
                  <!-- 竖双门 -->
                  <img
                    class="hdoubledoor"
                    src="http://39.105.179.38:9797/noderad/%E7%BE%A4%E7%BB%84%205%20Copy%2025@2x.png "
                  />
                </div>
                <div>
                  <div id="A2206" v-if="props.roomName=='A2-206'" :class="[(props.roomName=='A2-206') ? 'active-startA2206':'']">
                    A2-206<br />
                    音控室
                  </div>
                  <div id="A2206" v-else data-roomName="A2206">
                    A2-206<br />
                    音控室
                  </div>
                  <!-- 单门 左 -->
                  <img
                    class="door"
                    src="http://39.105.179.38:9797/noderad/%E7%BE%A4%E7%BB%84%202%20Copy.png "
                  />
                </div>
              </div>

              <div class="left2-2">
                <div></div>
                <div>
                  <div>
                    <div id="A2207" v-if="props.roomName=='A2-207'" :class="[(props.roomName=='A2-207') ? 'active-startA2207':'']">A2-207</div>
                    <div id="A2207" v-else data-roomName="A2207">A2-207</div>
                    <!-- 单扇门 右  左 -->
                    <img
                      class="door"
                      src="http://39.105.179.38:9797/noderad/%E7%BE%A4%E7%BB%84%20Copy%203@.png"
                    />
                    <img
                      class="door"
                      src="http://39.105.179.38:9797/noderad/%E7%BE%A4%E7%BB%84%202%20Copy.png "
                    />
                  </div>
                  <div>
                    <div id="A2208" v-if="props.roomName=='A2-208'" :class="[(props.roomName=='A2-208') ? 'active-startA2208':'']">A2-208</div>
                    <div id="A2208" v-else data-roomName="A2208">A2-208</div>
                    <!-- 单扇门 下  上 -->
                    <img
                      class="door"
                      src="http://39.105.179.38:9797/noderad/%E7%BE%A4%E7%BB%84%202%20Copy%206@2x%20-%20%E5%89%AF%E6%9C%AC.png"
                    />
                    <img
                      class="door"
                      src="http://39.105.179.38:9797/noderad/%E7%BE%A4%E7%BB%84%20Copy%203@2x.png"
                    />
                  </div>
                </div>
              </div>
              <div class="left2-3">
                <div></div>
                <!-- 排风扇竖 -->
                <img src="http://39.105.179.38:9797/noderad/%E7%BE%A4%E7%BB%84%205@2x%20(1).png" />
                <div>
                  <img src="http://39.105.179.38:9797/noderad/huoti.png" />
                </div>
              </div>
            </div>
            <div class="left3">过&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;道</div>
            <!-- 会议室2区 -->
            <div class="left4">
              <div class="left4-1">
                <div>
                  <div id="A2212" data-roomName="A2212">A2-212</div>
                  <!-- 竖 双排门 -->
                  <img
                    class="hdoubledoor"
                    src="http://39.105.179.38:9797/noderad/%E7%BE%A4%E7%BB%84%205%20Copy%2025@2x.png"
                  />
                  <img
                    class="hdoubledoor"
                    src="http://39.105.179.38:9797/noderad/%E7%BE%A4%E7%BB%84%205%20Copy%2025@2x.png"
                  />
                </div>
                <div></div>
              </div>
              <div class="left4-2">
                <div>
                  <div id="A2205" data-roomName="A2205">A2-205</div>
                  <!-- 横 双门 -->
                  <img
                    class="doubledoor"
                    src="http://39.105.179.38:9797/noderad/%E7%BE%A4%E7%BB%84%205%20Copy%204@2x.png"
                  />
                  <img
                    class="doubledoor"
                    src="http://39.105.179.38:9797/noderad/%E7%BE%A4%E7%BB%84%205%20Copy%204@2x.png"
                  />
                </div>
                <div>
                  <div id="A2201" v-if="props.roomName=='A2-201'" :class="[(props.roomName=='A2-201') ? 'active-startA2201':'']">A2-201</div>
                  <div id="A2201" v-else data-roomName="A2201">A2-201</div>
                  <img
                    class="doubledoor"
                    src="http://39.105.179.38:9797/noderad/%E7%BE%A4%E7%BB%84%205%20Copy%204@2x.png"
                  />
                  <img
                    class="doubledoor"
                    src="http://39.105.179.38:9797/noderad/%E7%BE%A4%E7%BB%84%205%20Copy%204@2x.png"
                  />
                </div>
              </div>
              <div class="left4-3">
                <div>
                  <div id="A2204" data-roomName="A2204">A2-204</div>
                  <!-- 横 双排门 -->
                  <img
                    class="doubledoor"
                    src="http://39.105.179.38:9797/noderad/%E7%BE%A4%E7%BB%84%205%20Copy%204@2x.png"
                  />
                </div>
                <div>
                  <div id="A2202" v-if="props.roomName=='A2-202'" :class="[(props.roomName=='A2-202') ? 'active-startA2202':'']">
                    <br />A2-202<br />
                    服务室
                  </div>
                  <div id="A2202" v-else data-roomName="A2202">
                    <br />A2-202<br />
                    服务室
                  </div>
                  <img
                    class="doubledoor"
                    src="http://39.105.179.38:9797/noderad/%E7%BE%A4%E7%BB%84%205%20Copy%204@2x.png"
                  />
                </div>
              </div>
              <div class="left4-4">
                <div></div>
                <img src="http://39.105.179.38:9797/noderad/%E7%BE%A4%E7%BB%84%205@2x%20(1).png" />
              </div>
            </div>
            <div class="left5">
              <div>
                <div>
                  <div>
                    <!-- 需改图片 -->
                    <img src="http://39.105.179.38:9797/noderad/iconPark-girl.png" />
                  </div>
                  <div>
                    <!-- 需改图片 -->
                    <img src="http://39.105.179.38:9797/noderad/iconPark-boy.png" />
                  </div>
                </div>
                <div></div>
              </div>
              <div>过&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;道</div>
            </div>
          </div>
          <div class="middle">
            <div>
              <div>
                <img src="http://39.105.179.38:9797/noderad/dianti.png" />
                <img src="http://39.105.179.38:9797/noderad/dianti.png" />
                <img src="http://39.105.179.38:9797/noderad/huoti.png" />
              </div>
              <!-- 排风扇 竖 -->
              <img src="http://39.105.179.38:9797/noderad/%E7%BE%A4%E7%BB%84%205@2x%20(1).png" />
              <img src="http://39.105.179.38:9797/noderad/%E7%BE%A4%E7%BB%84%205@2x%20(1).png" />
            </div>
            <div>
              <div>
                <div>
                  <img src="http://39.105.179.38:9797/noderad/shoufuti2.png" />
                </div>
              </div>
            </div>
          </div>
          <div class="right">
            <div class="right1">
              <div>
                <div></div>
                <div>
                  <div>
                    <!-- 需改图片 -->
                    <img src="http://39.105.179.38:9797/noderad/peidianjian.png" />
                  </div>
                  <div>
                    <!-- 需改图片 -->
                    <img src="http://39.105.179.38:9797/noderad/peidianjian.png" />
                  </div>
                </div>
              </div>
              <div>过&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;道</div>
            </div>
            <div class="right2">
              <div class="right2-1">
                <div>
                  <div id="A2229" data-roomName="A2229">A2-229</div>
                  <!-- 竖 双排门 -->
                  <img
                    class="hdoubledoor"
                    src="http://39.105.179.38:9797/noderad/%E7%BE%A4%E7%BB%84%205%20Copy%2025@2x.png"
                  />
                  <img
                    class="hdoubledoor"
                    src="http://39.105.179.38:9797/noderad/%E7%BE%A4%E7%BB%84%205%20Copy%2025@2x.png"
                  />
                </div>
                <div>
                  <div id="A2221" v-if="props.roomName=='A2-221'" :class="[(props.roomName=='A2-221') ? 'active-startA2221':'']">A2-221</div>
                  <div id="A2221" v-else data-roomName="A2221">A2-221</div>
                  <!-- 横 双门 -->
                  <img
                    class="doubledoor"
                    src="http://39.105.179.38:9797/noderad/%E7%BE%A4%E7%BB%84%205%20Copy%204@2x.png"
                  />
                </div>
              </div>
              <div class="right2-2">
                <div>
                  <div id="A2215" data-roomName="A2215">A2-215</div>
                  <!-- 横 双门 -->
                  <img
                    class="doubledoor"
                    src="http://39.105.179.38:9797/noderad/%E7%BE%A4%E7%BB%84%205%20Copy%204@2x.png"
                  />
                  <img
                    class="doubledoor"
                    src="http://39.105.179.38:9797/noderad/%E7%BE%A4%E7%BB%84%205%20Copy%204@2x.png"
                  />
                </div>
                <div>
                  <div id="A2220" v-if="props.roomName=='A2-220'" :class="[(props.roomName=='A2-220') ? 'active-startA2220':'']">A2-220</div>
                    <div id="A2220" v-else data-roomName="A2220">A2-220</div>
                  <img
                    class="doubledoor"
                    src="http://39.105.179.38:9797/noderad/%E7%BE%A4%E7%BB%84%205%20Copy%204@2x.png"
                  />
                  <img
                    class="doubledoor"
                    src="http://39.105.179.38:9797/noderad/%E7%BE%A4%E7%BB%84%205%20Copy%204@2x.png"
                  />
                </div>
              </div>
              <div class="right2-3">
                <div>
                  <div id="A2216" data-roomName="A2216"><br />A2-216<br />工作室</div>
                  <!-- 单扇门 右  左 -->
                  <img
                    class="door"
                    src="http://39.105.179.38:9797/noderad/%E7%BE%A4%E7%BB%84%20Copy%203@.png"
                  />
                  <img
                    class="door"
                    src="http://39.105.179.38:9797/noderad/%E7%BE%A4%E7%BB%84%202%20Copy.png "
                  />
                </div>
                <div>
                  <div id="A2219" v-if="props.roomName=='A2-219'" :class="[(props.roomName=='A2-219') ? 'active-startA2219':'']">A2-219</div>
                    <div id="A2219" v-else data-roomName="A2219">A2-219</div>
                  <img
                    class="doubledoor"
                    src="http://39.105.179.38:9797/noderad/%E7%BE%A4%E7%BB%84%205%20Copy%204@2x.png"
                  />
                </div>
              </div>
              <div class="right2-4">
                <img src="http://39.105.179.38:9797/noderad/%E7%BE%A4%E7%BB%84%205@2x%20(1).png" />
                <div></div>
              </div>
            </div>
            <div class="right3">过&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;道</div>
            <div class="right4">
              <div class="right4-1">
                <div>
                  <div id="A2228" data-roomName="A2228">A2-228</div>
                  <!-- 竖 双排门 -->
                  <img
                    class="hdoubledoor"
                    src="http://39.105.179.38:9797/noderad/%E7%BE%A4%E7%BB%84%205%20Copy%2025@2x.png"
                  />
                  <img
                    class="hdoubledoor"
                    src="http://39.105.179.38:9797/noderad/%E7%BE%A4%E7%BB%84%205%20Copy%2025@2x.png"
                  />
                </div>
                <div>
                  <div>
                    <img
                      src="http://39.105.179.38:9797/noderad/%E5%9B%BE%E7%89%87%202@2x%20(1).png"
                    />
                    <img
                      src="http://39.105.179.38:9797/noderad/%E5%9B%BE%E7%89%87%202@2x%20(1).png"
                    />
                    <img
                      src="http://39.105.179.38:9797/noderad/%E5%9B%BE%E7%89%87%202@2x%20(1).png"
                    />
                  </div>
                </div>
              </div>
              <div class="right4-2">
                <div>
                  <div>
                    <div id="A2222" data-roomName="A2222">A2-222</div>
                    <!-- 横 双排门 -->
                    <img
                      class="doubledoor"
                      src="http://39.105.179.38:9797/noderad/%E7%BE%A4%E7%BB%84%205%20Copy%204@2x.png"
                    />
                  </div>
                  <div>
                    <div id="A2223" data-roomName="A2223">A2-223</div>
                    <!-- 横 双排门 -->
                    <img
                      class="doubledoor"
                      src="http://39.105.179.38:9797/noderad/%E7%BE%A4%E7%BB%84%205%20Copy%204@2x.png"
                    />
                  </div>
                </div>
                <div>
                  <div>
                    <div id="A2227" data-roomName="A2227">A2-227</div>
                    <!-- 横 双排门 -->
                    <img
                      class="doubledoor"
                      src="http://39.105.179.38:9797/noderad/%E7%BE%A4%E7%BB%84%205%20Copy%204@2x.png"
                    />
                  </div>
                  <div>
                    <div id="A2226" v-if="props.roomName=='A2-226'"  :class="[(props.roomName=='A2-226') ? 'active-start':'']">A2-226</div>
                    <div id="A2226" v-else data-roomName="A2226" >A2-226</div>
                    <!-- 横 双排门 -->
                    <img
                      class="doubledoor"
                      src="http://39.105.179.38:9797/noderad/%E7%BE%A4%E7%BB%84%205%20Copy%204@2x.png"
                    />
                  </div>
                  <div>
                    <div id="A2225" v-if="props.roomName=='A2-225'" :class="[(props.roomName=='A2-225') ? 'active-start':'']">A2-225</div>
                    <div id="A2225" v-else data-roomName="A2225">A2-225</div>
                    <!-- 横 双排门 -->
                    <img
                      class="doubledoor"
                      src="http://39.105.179.38:9797/noderad/%E7%BE%A4%E7%BB%84%205%20Copy%204@2x.png"
                    />
                  </div>
                </div>
              </div>
              <div class="right4-3">
                <div></div>
                <img src="http://39.105.179.38:9797/noderad/%E7%BE%A4%E7%BB%84%205@2x%20(1).png" />
              </div>
            </div>
            <div class="right5">
              <div>过&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;道</div>
            </div>
          </div>
        </div>
        <!-- 过道 -->
        <div class="div2-3" id="div2-3"><br />过<br />道 <br /><br /><br />过<br />道</div>
        <div class="arrowPoint" id="zuobiao" :style="getzbdStyle">
          <span>左</span>
          <img src="http://39.105.179.38:9797/noderad/md-forward@2x.png" />

          <img id="zuobiaodian" src="http://39.105.179.38:9797/noderad/zhishi.png" />
          <img src="http://39.105.179.38:9797/noderad/md-forward@2x%20(1).png" />
          <span>右</span>
        </div>
      </div>
    </div>
    <div class="div1">
      <ul>
        <li><img src="http://39.105.179.38:9797/noderad/dianti.png" /><span>电梯</span></li>
        <li><img src="http://39.105.179.38:9797/noderad/louti.png" /><span>楼梯</span></li>
        <li><img src="http://39.105.179.38:9797/noderad/shoufudt.png" /><span>手扶电梯</span></li>
        <li><img src="http://39.105.179.38:9797/noderad/huoti.png" /><span>货梯</span></li>
        <li><img src="http://39.105.179.38:9797/noderad/wc.png" /><span>卫生间</span></li>
        <li><img src="http://39.105.179.38:9797/noderad/peidianjian.png" /><span>配电间</span></li>
        <li>
          <!-- <img
                              src="http://39.105.179.38:9797/noderad/dangqian.png"/> -->
          <span class="zhongdian"></span><span>当前所在位置</span>
        </li>
      </ul>
    </div>
  </div>
</template>
<script setup>
import { ref, reactive, watch, computed, onMounted, nextTick } from 'vue'
import $ from 'jquery'
const props=defineProps({
    roomName:{
        type:String,
        default(){
            return ''
        }
    },
    form:{
      type:Object,
        default(){
            return ''
        }
    }
})
// 二楼竖正：206无、  207、208、201、  202无、    221、220、219、227、226、225  ok
const getzbdStyle=computed(()=>{
    if(props.roomName=="A2-227"){
      return {
       left:"140px",
       top:"1014px",
      }
    }
    if(props.roomName=="A2-226"){
      return {
       left:"220px",
       top:"1014px",
      }
    }
    if(props.roomName=="A2-225"){
      return {
       left:"284px",
       top:"1014px",
      }
    }
    if(props.roomName=="A2-219"){
      return {
       left:"274px",
       top:"804px",
      }
    }
    if(props.roomName=="A2-220"){
      return {
       left:"200px",
       top:"804px",
      }
    }
    if(props.roomName=="A2-221"){
      return {
       left:"48px",
       top:"804px",
      }
    }
    if(props.roomName=="A2-202"){
      return {
       left:"276px",
       top:"444px",
      }
    }
    if(props.roomName=="A2-201"){
      return {
       left:"204px",
       top:"444px",
      }
    }
    if(props.roomName=="A2-208"){
      return {
       left:"294px",
       top:"224px",
      }
    }
    if(props.roomName=="A2-207"){
      return {
       left:"188px",
       top:"224px",
      }
    }
    if(props.roomName=="A2-206"){
      return {
       left:"80px",
       top:"224px",
      }
    }
})


onMounted(() => {
  nextTick(() => {
    let ele = document.getElementById('div2cont');
    ele.scrollTop = 984;
    
    var scrolltimer
    var type = 1
    function scrollSwiper() {
      // debugger
      scrolltimer = setInterval(function () {
        if (type == 3) {
          type = 1
        } else {
          type++
        }
        if (type == 1) {
          $('#div2').css('zoom', 1.972)
          ele.scrollTop = 984
        } else if (type == 2) {
          $('#div2').css('zoom', 1.972)
          ele.scrollTop = 0
        } else {
          $('#div2').css('zoom', 1)
        }
      }, 500000000)
    }
    scrollSwiper()
    // 声明会议室坐标点
    var CoordPoint = {
      // A2-225会议室
      // 一区
      A2225_A2208: [
        [344, 1020],
        [447, 1020],
        [442, 1020],
        [442, 234],
        [442, 234],
        [354, 234]
      ],
      A2225_A2207: [
        [344, 1020],
        [447, 1020],
        [442, 1020],
        [442, 234],
        [447, 234],
        [248, 234]
      ],
      A2225_A2211: [
        [344, 1020],
        [38, 1020],
        [38, 1020],
        [38, 130]
      ],
      A2225_A2206: [
        [344, 1020],
        [38, 1020],
        [38, 1020],
        [38, 234],
        [38, 234],
        [140, 234]
      ],
      //二区
      A2225_A2204: [
        [344, 1020],
        [447, 1020],
        [442, 1020],
        [442, 234],
        [442, 234],
        [340, 234]
      ],
      A2225_A2205: [
        [344, 1020],
        [447, 1020],
        [442, 1020],
        [442, 234],
        [442, 234],
        [262, 234]
      ],
      A2225_A2202: [
        [344, 1020],
        [447, 1020],
        [442, 1020],
        [442, 454],
        [442, 454],
        [336, 454]
      ],
      A2225_A2201: [
        [344, 1020],
        [447, 1020],
        [442, 1020],
        [442, 454],
        [442, 454],
        [264, 454]
      ],
      A2225_A2212: [
        [344, 1020],
        [38, 1020],
        [38, 1020],
        [38, 348]
      ],
      // //3区
      A2225_A2216: [
        [344, 1020],
        [447, 1020],
        [442, 1020],
        [442, 582],
        [447, 582],
        [356, 582]
      ],
      A2225_A2215: [
        [344, 1020],
        [447, 1020],
        [442, 1020],
        [442, 582],
        [447, 582],
        [260, 582]
      ],
      A2225_A2229: [
        [344, 1020],
        [38, 1020],
        [38, 1020],
        [38, 690]
      ],
      A2225_A2219: [
        [344, 1020],
        [447, 1020],
        [442, 1020],
        [442, 814],
        [447, 814],
        [334, 814]
      ],
      A2225_A2220: [
        [344, 1020],
        [447, 1020],
        [442, 1020],
        [442, 814],
        [447, 814],
        [260, 814]
      ],
      A2225_A2221: [
        [344, 1020],
        [38, 1020],
        [38, 1020],
        [38, 814],
        [38, 814],
        [108, 814]
      ],

      // //4区
      A2225_A2223: [
        [344, 1020],
        [447, 1020],
        [442, 1020],
        [442, 814],
        [447, 814],
        [350, 814]
      ],
      A2225_A2222: [
        [344, 1020],
        [447, 1020],
        [442, 1020],
        [442, 814],
        [447, 814],
        [246, 814]
      ],
      A2225_A2228: [
        [344, 1020],
        [38, 1020],
        [38, 1020],
        [38, 930]
      ],
      A2225_A2225: [
        // [344, 1020],
        // [344, 1020],
      ],
      A2225_A2226: [
        [344, 1020],
        [276, 1020]
      ],
      A2225_A2227: [
        [344, 1020],
        [196, 1020]
      ],

       
      // A2-226会议室
      // 一区
      A2226_A2208: [
          [278, 1020],
          [447, 1020],
          [442, 1020],
          [442, 234],
          [442, 234],
          [354, 234],
      ],
      A2226_A2207: [
          [278, 1020],
          [447, 1020],
          [442, 1020],
          [442, 234],
          [447, 234],
          [248, 234],
      ],
      A2226_A2211: [
          [278, 1020],
          [38, 1020],
          [38, 1020],
          [38, 130],
      ],
      A2226_A2206: [
          [278, 1020],
          [38, 1020],
          [38, 1020],
          [38, 234],
          [38, 234],
          [140, 234],
      ],
      //二区
      A2226_A2204: [
          [278, 1020],
          [447, 1020],
          [442, 1020],
          [442, 234],
          [442, 234],
          [340, 234],
      ],
      A2226_A2205: [
          [278, 1020],
          [447, 1020],
          [442, 1020],
          [442, 234],
          [442, 234],
          [262, 234],
      ],
      A2226_A2202: [
          [278, 1020],
          [447, 1020],
          [442, 1020],
          [442, 454],
          [442, 454],
          [336, 454],
      ],
      A2226_A2201: [
          [278, 1020],
          [447, 1020],
          [442, 1020],
          [442, 454],
          [442, 454],
          [264, 454],
      ],
      A2226_A2212: [
          [278, 1020],
          [38, 1020],
          [38, 1020],
          [38, 348],
      ],
      // //3区
      A2226_A2216: [
          [278, 1020],
          [447, 1020],
          [442, 1020],
          [442, 582],
          [447, 582],
          [356, 582],
      ],
      A2226_A2215: [
          [278, 1020],
          [447, 1020],
          [442, 1020],
          [442, 582],
          [447, 582],
          [260, 582],
      ],
      A2226_A2229: [
          [278, 1020],
          [38, 1020],
          [38, 1020],
          [38, 690],
      ],
      A2226_A2219: [
          [278, 1020],
          [447, 1020],
          [442, 1020],
          [442, 814],
          [447, 814],
          [334, 814],
      ],
      A2226_A2220: [
          [278, 1020],
          [447, 1020],
          [442, 1020],
          [442, 814],
          [447, 814],
          [260, 814],
      ],
      A2226_A2221: [
          [278, 1020],
          [38, 1020],
          [38, 1020],
          [38, 814],
          [38, 814],
          [108, 814],
      ],

      // //4区
      A2226_A2223: [
          [278, 1020],
          [447, 1020],
          [442, 1020],
          [442, 814],
          [447, 814],
          [350, 814],
      ],
      A2226_A2222: [
          [278, 1020],
          [447, 1020],
          [442, 1020],
          [442, 814],
          [447, 814],
          [246, 814],
      ],
      A2226_A2228: [
          [278, 1020],
          [38, 1020],
          [38, 1020],
          [38, 930],
      ],
      A2226_A2225: [
          [278, 1020],
          [344, 1020],
      ],
      A2226_A2226: [

      ],
      A2226_A2227: [
          [278, 1020],
          [196, 1020],
      ],

      // A2-227
      // 一区
      A2227_A2208: [
          [196, 1020],
          [447, 1020],
          [442, 1020],
          [442, 234],
          [442, 234],
          [354, 234],
      ],
      A2227_A2207: [
          [196, 1020],
          [38, 1020],
          [38, 1020],
          [38, 234],
          [38, 234],
          [248, 234],
      ],
      A2227_A2211: [
          [196, 1020],
          [38, 1020],
          [38, 1020],
          [38, 130],
      ],
      A2227_A2206: [
          [196, 1020],
          [38, 1020],
          [38, 1020],
          [38, 234],
          [38, 234],
          [140, 234],
      ],
      //二区
      A2227_A2204: [
          [196, 1020],
          [447, 1020],
          [442, 1020],
          [442, 234],
          [442, 234],
          [340, 234],
      ],
      A2227_A2205: [
          [196, 1020],
          [447, 1020],
          [442, 1020],
          [442, 234],
          [442, 234],
          [262, 234],
      ],
      A2227_A2202: [
          [196, 1020],
          [447, 1020],
          [442, 1020],
          [442, 454],
          [442, 454],
          [336, 454],
      ],
      A2227_A2201: [
          [196, 1020],
          [447, 1020],
          [442, 1020],
          [442, 454],
          [442, 454],
          [264, 454],
      ],
      A2227_A2212: [
          [196, 1020],
          [38, 1020],
          [38, 1020],
          [38, 348],
      ],
      // //3区
      A2227_A2216: [
          [196, 1020],
          [447, 1020],
          [442, 1020],
          [442, 582],
          [447, 582],
          [356, 582],
      ],
      A2227_A2215: [
          [196, 1020],
          [447, 1020],
          [442, 1020],
          [442, 582],
          [447, 582],
          [260, 582],
      ],
      A2227_A2229: [
          [196, 1020],
          [38, 1020],
          [38, 1020],
          [38, 690],
      ],
      A2227_A2219: [
          [196, 1020],
          [447, 1020],
          [442, 1020],
          [442, 814],
          [447, 814],
          [334, 814],
      ],
      A2227_A2220: [
          [196, 1020],
          [447, 1020],
          [442, 1020],
          [442, 814],
          [447, 814],
          [260, 814],
      ],
      A2227_A2221: [
          [196, 1020],
          [38, 1020],
          [38, 1020],
          [38, 814],
          [38, 814],
          [108, 814],
      ],

      // //4区
      A2227_A2223: [
          [196, 1020],
          [447, 1020],
          [442, 1020],
          [442, 814],
          [447, 814],
          [350, 814],
      ],
      A2227_A2222: [
          [196, 1020],
          [38, 1020],
          [38, 1020],
          [38, 814],
          [38, 814],
          [246, 814],
      ],
      A2227_A2228: [
          [196, 1020],
          [38, 1020],
          [38, 1020],
          [38, 930],
      ],
      A2227_A2225: [
          [196, 1020],
          [344, 1020],
      ],
      A2227_A2226: [
          [196, 1020],
          [276, 1020],
      ],
      A2227_A2227: [

      ],
      
      // A2-219
      // 一区
    A2219_A2208: [
        [334, 814],
        [447, 814],
        [442, 814],
        [442, 234],
        [447, 234],
        [354, 234],
    ],
    A2219_A2207: [
        [334, 814],
        [447, 814],
        [442, 814],
        [442, 234],
        [447, 234],
        [248, 234],
    ],
    A2219_A2211: [
        [334, 814],
        [38, 814],
        [38, 814],
        [38, 126],
    ],
    A2219_A2206: [
        [334, 814],
        [38, 814],
        [38, 814],
        [38, 234],
        [38, 234],
        [140, 234],
    ],
    //二区
    A2219_A2204: [
        [334, 814],
        [447, 814],
        [442, 814],
        [442, 234],
        [447, 234],
        [340, 234],
    ],
    A2219_A2205: [
        [334, 814],
        [447, 814],
        [442, 814],
        [442, 234],
        [447, 234],
        [262, 234],
    ],
    A2219_A2202: [
        [334, 814],
        [447, 814],
        [442, 814],
        [442, 454],
        [447, 454],
        [336, 454],
    ],
    A2219_A2201: [
        [334, 814],
        [447, 814],
        [442, 814],
        [442, 454],
        [447, 454],
        [264, 454],
    ],
    A2219_A2212: [
        [334, 814],
        [38, 814],
        [38, 814],
        [38, 342],
    ],
    // //3区
    A2219_A2216: [
        [334, 814],
        [447, 814],
        [442, 814],
        [442, 588],
        [447, 588],
        [356, 588],
    ],
    A2219_A2215: [
        [334, 814],
        [447, 814],
        [442, 814],
        [442, 588],
        [447, 588],
        [260, 588],
    ],
    A2219_A2229: [
        [334, 814],
        [38, 814],
        [38, 814],
        [38, 690],
    ],
    A2219_A2219: [

    ],
    A2219_A2220: [
        [334, 814],
        [256, 814],
    ],
    A2219_A2221: [
        [334, 814],
        [100, 814],
    ],

    //4区
    A2219_A2223: [
        [334, 814],
        [346, 814],
    ],
    A2219_A2222: [
        [334, 814],
        [242, 814],
    ],
    A2219_A2228: [
        [334, 814],
        [38, 814],
        [38, 814],
        [38, 930],
    ],
    A2219_A2225: [
        [334, 814],
        [447, 814],
        [442, 814],
        [442, 1020],
        [447, 1020],
        [348, 1020],
    ],
    A2219_A2226: [
        [334, 814],
        [447, 814],
        [442, 814],
        [442, 1020],
        [447, 1020],
        [280, 1020],
    ],
    A2219_A2227: [
        [334, 814],
        [447, 814],
        [442, 814],
        [442, 1020],
        [447, 1020],
        [196, 1020],
    ],
     
    // A2-220-------------------
     // 一区
    A2220_A2208: [
        [256, 814],
        [447, 814],
        [442, 814],
        [442, 234],
        [447, 234],
        [354, 234],
    ],
    A2220_A2207: [
        [256, 814],
        [447, 814],
        [442, 814],
        [442, 234],
        [447, 234],
        [248, 234],
    ],
    A2220_A2211: [
        [256, 814],
        [38, 814],
        [38, 814],
        [38, 126],
    ],
    A2220_A2206: [
        [256, 814],
        [38, 814],
        [38, 814],
        [38, 234],
        [38, 234],
        [140, 234],
    ],
    //二区
    A2220_A2204: [
        [256, 814],
        [447, 814],
        [442, 814],
        [442, 234],
        [447, 234],
        [340, 234],
    ],
    A2220_A2205: [
        [256, 814],
        [447, 814],
        [442, 814],
        [442, 234],
        [447, 234],
        [262, 234],
    ],
    A2220_A2202: [
        [256, 814],
        [447, 814],
        [442, 814],
        [442, 454],
        [447, 454],
        [336, 454],
    ],
    A2220_A2201: [
        [256, 814],
        [447, 814],
        [442, 814],
        [442, 454],
        [447, 454],
        [264, 454],
    ],
    A2220_A2212: [
        [256, 814],
        [38, 814],
        [38, 814],
        [38, 342],
    ],
    // //3区
    A2220_A2216: [
        [256, 814],
        [447, 814],
        [442, 814],
        [442, 588],
        [447, 588],
        [356, 588],
    ],
    A2220_A2215: [
        [256, 814],
        [447, 814],
        [442, 814],
        [442, 588],
        [447, 588],
        [260, 588],
    ],
    A2220_A2229: [
        [256, 814],
        [38, 814],
        [38, 814],
        [38, 690],
    ],
    A2220_A2219: [
        [256, 814],
        [334, 814],
    ],
    A2220_A2220: [

    ],
    A2220_A2221: [
        [256, 814],
        [100, 814],
    ],

    //4区
    A2220_A2223: [
        [256, 814],
        [346, 814],
    ],
    A2220_A2222: [
        [256, 814],
        [242, 814],
    ],
    A2220_A2228: [
        [256, 814],
        [38, 814],
        [38, 814],
        [38, 930],
    ],
    A2220_A2225: [
        [256, 814],
        [447, 814],
        [442, 814],
        [442, 1020],
        [447, 1020],
        [348, 1020],
    ],
    A2220_A2226: [
        [256, 814],
        [447, 814],
        [442, 814],
        [442, 1020],
        [447, 1020],
        [280, 1020],
    ],
    A2220_A2227: [
        [256, 814],
        [38, 814],
        [38, 814],
        [38, 1020],
        [38, 1020],
        [196, 1020],
    ],
    
    // A2-221-----------------
    // 一区
    A2221_A2208: [
        [100, 814],
        [447, 814],
        [442, 814],
        [442, 234],
        [447, 234],
        [354, 234],
    ],
    A2221_A2207: [
        [100, 814],
        [38, 814],
        [38, 814],
        [38, 234],
        [38, 234],
        [248, 234],
    ],
    A2221_A2211: [
        [100, 814],
        [38, 814],
        [38, 814],
        [38, 126],
    ],
    A2221_A2206: [
        [100, 814],
        [38, 814],
        [38, 814],
        [38, 234],
        [38, 234],
        [140, 234],
    ],
    //二区
    A2221_A2204: [
        [100, 814],
        [447, 814],
        [442, 814],
        [442, 234],
        [447, 234],
        [340, 234],
    ],
    A2221_A2205: [
        [100, 814],
        [38, 814],
        [38, 814],
        [38, 234],
        [38, 234],
        [262, 234],
    ],
    A2221_A2202: [
        [100, 814],
        [447, 814],
        [442, 814],
        [442, 454],
        [447, 454],
        [336, 454],
    ],
    A2221_A2201: [
        [100, 814],
        [447, 814],
        [442, 814],
        [442, 454],
        [447, 454],
        [264, 454],
    ],
    A2221_A2212: [
        [100, 814],
        [38, 814],
        [38, 814],
        [38, 342],
    ],
    // //3区
    A2221_A2216: [
        [100, 814],
        [447, 814],
        [442, 814],
        [442, 588],
        [447, 588],
        [356, 588],
    ],
    A2221_A2215: [
        [100, 814],
        [447, 814],
        [442, 814],
        [442, 588],
        [447, 588],
        [260, 588],
    ],
    A2221_A2229: [
        [100, 814],
        [38, 814],
        [38, 814],
        [38, 690],
    ],
    A2221_A2219: [
        [100, 814],
        [334, 814],
    ],
    A2221_A2220: [
        [100, 814],
        [200, 814],
    ],
    A2221_A2221: [

    ],

    //4区
    A2221_A2223: [
        [100, 814],
        [346, 814],
    ],
    A2221_A2222: [
        [100, 814],
        [242, 814],
    ],
    A2221_A2228: [
        [100, 814],
        [38, 814],
        [38, 814],
        [38, 930],
    ],
    A2221_A2225: [
        [100, 814],
        [447, 814],
        [442, 814],
        [442, 1020],
        [447, 1020],
        [348, 1020],
    ],
    A2221_A2226: [
        [100, 814],
        [38, 814],
        [38, 814],
        [38, 1020],
        [38, 1020],
        [280, 1020],
    ],
    A2221_A2227: [
        [100, 814],
        [38, 814],
        [38, 814],
        [38, 1020],
        [38, 1020],
        [196, 1020],
    ],

    // A2-201--------
    // 一区
    A2201_A2208: [
        [258, 454],
        [447, 454],
        [442, 454],
        [442, 234],
        [447, 234],
        [354, 234],
    ],
    A2201_A2207: [
        [258, 454],
        [447, 454],
        [442, 454],
        [442, 234],
        [447, 234],
        [248, 234],
    ],
    A2201_A2211: [
        [258, 454],
        [258, 489],
        [263, 484],
        [38, 484],
        [38, 484],
        [38, 126],
    ],
    A2201_A2206: [
        [258, 454],
        [258, 489],
        [263, 484],
        [38, 484],
        [38, 484],
        [38, 234],
        [43, 234],
        [134, 234],
    ],
    //二区
    A2201_A2204: [
        [258, 454],
        [447, 454],
        [442, 454],
        [442, 234],
        [447, 234],
        [340, 234],
    ],
    A2201_A2205: [
        [258, 454],
        [447, 454],
        [442, 454],
        [442, 234],
        [447, 234],
        [262, 234],
    ],
    A2201_A2202: [
        [258, 454],
        [334, 454],
    ],
    A2201_A2201: [
       
    ],
    A2201_A2212: [
        [258, 454],
        [258, 489],
        [263, 484],
        [38, 484],
        [38, 484],
        [38, 342],
    ],
    // //3区
    A2201_A2216: [
        [258, 454],
        [447, 454],
        [442, 454],
        [442, 580],
        [447, 580],
        [356, 580],
    ],
    A2201_A2215: [
        [258, 454],
        [258, 580],
    ],
    A2201_A2229: [
        [200, 454],
        [200, 565],
        [200, 560],
        [38, 560],
        [38, 560],
        [38, 640],
    ],
    A2201_A2219: [
        [258, 454],
        [447, 454],
        [442, 454],
        [442, 814],
        [447, 814],
        [334, 814],
    ],
    A2201_A2220: [
        [258, 454],
        [447, 454],
        [442, 454],
        [442, 814],
        [447, 814],
        [260, 814],
    ],
    A2201_A2221: [
        [200, 454],
        [200, 565],
        [200, 560],
        [38, 560],
        [38, 560],
        [38, 814],
        [38, 814],
        [104, 814],
    ],

    // //4区
    A2201_A2223: [
        [258, 454],
        [447, 454],
        [442, 454],
        [442, 814],
        [447, 814],
        [350, 814],
    ],
    A2201_A2222: [
        [258, 454],
        [447, 454],
        [442, 454],
        [442, 814],
        [447, 814],
        [246, 814],
    ],
    A2201_A2228: [
        [200, 454],
        [200, 565],
        [200, 560],
        [38, 560],
        [38, 560],
        [38, 870],
    ],
    A2201_A2225: [
        [258, 454],
        [447, 454],
        [442, 454],
        [442, 1020],
        [447, 1020],
        [348, 1020],
    ],
    A2201_A2226: [
        [258, 454],
        [447, 454],
        [442, 454],
        [442, 1020],
        [447, 1020],
        [280, 1020],
    ],
    A2201_A2227: [
        [200, 454],
        [200, 565],
        [200, 560],
        [38, 560],
        [38, 560],
        [38, 1020],
        [38, 1020],
        [196, 1020],
    ],

    // A2-202服务室--------
    // 一区
    A2202_A2208: [
        [331, 454],
        [447, 454],
        [442, 454],
        [442, 234],
        [447, 234],
        [354, 234],
    ],
    A2202_A2207: [
        [331, 454],
        [447, 454],
        [442, 454],
        [442, 234],
        [447, 234],
        [248, 234],
    ],
    A2202_A2211: [
        [331, 454],
        [331, 482],
        [336, 482],
        [38, 482],
        [38, 482],
        [38, 126],
    ],
    A2202_A2206: [
        [331, 454],
        [331, 482],
        [336, 482],
        [38, 482],
        [38, 482],
        [38, 234],
        [38, 234],
        [134, 234],
    ],
    //二区
    A2202_A2204: [
        [331, 454],
        [447, 454],
        [442, 454],
        [442, 234],
        [447, 234],
        [340, 234],
    ],
    A2202_A2205: [
        [331, 454],
        [447, 454],
        [442, 454],
        [442, 234],
        [447, 234],
        [262, 234],
    ],
    A2202_A2202: [
       
       
    ],
    A2202_A2201: [
        [331, 454],
        [257, 454],
    ],
    A2202_A2212: [
        [331, 454],
        [331, 482],
        [336, 482],
        [38, 482],
        [38, 482],
        [38, 342],
    ],
    // //3区
    A2202_A2216: [
        [331, 454],
        [447, 454],
        [442, 454],
        [442, 580],
        [447, 580],
        [356, 580],
    ],
    A2202_A2215: [
        [331, 454],
        [447, 454],
        [442, 454],
        [442, 580],
        [447, 580],
        [260, 580],
    ],
    A2202_A2229: [
        [331, 454],
        [331, 482],
        [336, 482],
        [38, 482],
        [38, 482],
        [38, 640],
    ],
    A2202_A2219: [
        [331, 454],
        [447, 454],
        [442, 454],
        [442, 814],
        [447, 814],
        [334, 814],
    ],
    A2202_A2220: [
        [331, 454],
        [447, 454],
        [442, 454],
        [442, 814],
        [447, 814],
        [260, 814],
    ],
    A2202_A2221: [
        [331, 454],
        [331, 482],
        [336, 482],
        [38, 482],
        [38, 482],
        [38, 814],
        [38, 814],
        [104, 814],
    ],

    // //4区
    A2202_A2223: [
        [331, 454],
        [447, 454],
        [442, 454],
        [442, 814],
        [447, 814],
        [350, 814],
    ],
    A2202_A2222: [
        [331, 454],
        [447, 454],
        [442, 454],
        [442, 814],
        [447, 814],
        [246, 814],
    ],
    A2202_A2228: [
        [331, 454],
        [331, 482],
        [336, 482],
        [38, 482],
        [38, 482],
        [38, 870],
    ],
    A2202_A2225: [
        [331, 454],
        [447, 454],
        [442, 454],
        [442, 1020],
        [447, 1020],
        [348, 1020],
    ],
    A2202_A2226: [
        [331, 454],
        [447, 454],
        [442, 454],
        [442, 1020],
        [447, 1020],
        [280, 1020],
    ],
    A2202_A2227: [
        [331, 454],
        [447, 454],
        [442, 454],
        [442, 1020],
        [447, 1020],
        [196, 1020],
    ],



    // A2-207--------
     // 一区
    A2207_A2208: [
        [244, 234],
        [304, 234],
    ],
    A2207_A2207: [
       
    ],
    A2207_A2211: [
        [244, 234],
        [38, 234],
        [38, 234],
        [38, 126],
    ],
    A2207_A2206: [
        [244, 234],
        [138, 234],
    ],
    //二区
    A2207_A2204: [
        [244, 234],
        [340, 234],
    ],
    A2207_A2205: [
        [244, 234],
        [262, 234],
    ],
    A2207_A2202: [
        [244, 234],
        [447, 234],
        [442, 234],
        [442, 454],
        [447, 454],
        [336, 454],
    ],
    A2207_A2201: [
        [244, 234],
        [447, 234],
        [442, 234],
        [442, 454],
        [447, 454],
        [264, 454],
    ],
    A2207_A2212: [
        [244, 234],
        [38, 234],
        [38, 234],
        [38, 342],
    ],
    // //3区
    A2207_A2216: [
        [244, 234],
        [447, 234],
        [442, 234],
        [442, 580],
        [447, 580],
        [356, 580],
    ],
    A2207_A2215: [
        [244, 234],
        [447, 234],
        [442, 234],
        [442, 580],
        [447, 580],
        [260, 580],
    ],
    A2207_A2229: [
        [244, 234],
        [38, 234],
        [38, 234],
        [38, 640],
    ],
    A2207_A2219: [
        [244, 234],
        [447, 234],
        [442, 234],
        [442, 814],
        [447, 814],
        [334, 814],
    ],
    A2207_A2220: [
        [244, 234],
        [447, 234],
        [442, 234],
        [442, 814],
        [447, 814],
        [260, 814],
    ],
    A2207_A2221: [
        [244, 234],
        [38, 234],
        [38, 234],
        [38, 814],
        [38, 814],
        [108, 814],
    ],

    // //4区
    A2207_A2223: [
        [244, 234],
        [447, 234],
        [442, 234],
        [442, 814],
        [447, 814],
        [350, 814],
    ],
    A2207_A2222: [
        [244, 234],
        [447, 234],
        [442, 234],
        [442, 814],
        [447, 814],
        [246, 814],
    ],
    A2207_A2228: [
        [244, 234],
        [38, 234],
        [38, 234],
        [38, 870],
    ],
    A2207_A2225: [
        [244, 234],
        [447, 234],
        [442, 234],
        [442, 1020],
        [447, 1020],
        [344, 1020],
    ],
    A2207_A2226: [
        [244, 234],
        [447, 234],
        [442, 234],
        [442, 1020],
        [447, 1020],
        [276, 1020],
    ],
    A2207_A2227: [
        [244, 234],
        [447, 234],
        [442, 234],
        [442, 1020],
        [447, 1020],
        [196, 1020],
    ],
    

    // A2-206--------
     // 一区
    A2206_A2208: [
        [135, 234],
        [300, 234],
    ],
    A2206_A2207: [
       [135, 234],
       [196, 234],
    ],
    A2206_A2211: [
        [135, 234],
        [38, 234],
        [38, 234],
        [38, 126],
    ],
    A2206_A2206: [
        
    ],
    //二区
    A2206_A2204: [
        [135, 234],
        [330, 234],
    ],
    A2206_A2205: [
        [135, 234],
        [198, 234],
    ],
    A2206_A2202: [
        [135, 234],
        [447, 234],
        [442, 234],
        [442, 454],
        [447, 454],
        [336, 454],
    ],
    A2206_A2201: [
        [135, 234],
        [447, 234],
        [442, 234],
        [442, 454],
        [447, 454],
        [264, 454],
    ],
    A2206_A2212: [
        [135, 234],
        [38, 234],
        [38, 234],
        [38, 342],
    ],
    // //3区
    A2206_A2216: [
        [135, 234],
        [447, 234],
        [442, 234],
        [442, 580],
        [447, 580],
        [356, 580],
    ],
    A2206_A2215: [
        [135, 234],
        [447, 234],
        [442, 234],
        [442, 580],
        [447, 580],
        [260, 580],
    ],
    A2206_A2229: [
        [135, 234],
        [38, 234],
        [38, 234],
        [38, 640],
    ],
    A2206_A2219: [
        [135, 234],
        [38, 234],
        [38, 234],
        [38, 814],
        [38, 814],
        [334, 814],
    ],
    A2206_A2220: [
        [135, 234],
        [38, 234],
        [38, 234],
        [38, 814],
        [38, 814],
        [204, 814],
    ],
    A2206_A2221: [
        [135, 234],
        [38, 234],
        [38, 234],
        [38, 814],
        [38, 814],
        [108, 814],
    ],

    // //4区
    A2206_A2223: [
        [135, 234],
        [38, 234],
        [38, 234],
        [38, 814],
        [38, 814],
        [350, 814],
    ],
    A2206_A2222: [
        [135, 234],
        [38, 234],
        [38, 234],
        [38, 814],
        [38, 814],
        [246, 814],
    ],
    A2206_A2228: [
        [135, 234],
        [38, 234],
        [38, 234],
        [38, 870],
    ],
    A2206_A2225: [
        [135, 234],
        [38, 234],
        [38, 234],
        [38, 1020],
        [38, 1020],
        [348, 1020],
    ],
    A2206_A2226: [
        [135, 234],
        [38, 234],
        [38, 234],
        [38, 1020],
        [38, 1020],
        [280, 1020],
    ],
    A2206_A2227: [
        [135, 234],
        [38, 234],
        [38, 234],
        [38, 1020],
        [38, 1020],
        [198, 1020],
    ],


    // A2-208-------------
    // 一区
    A2208_A2208: [

    ],
    A2208_A2207: [
        [300, 234],
        [244, 234],
    ],
    A2208_A2211: [
        [300, 234],
        [38, 234],
        [38, 234],
        [38, 126],
    ],
    A2208_A2206: [
        [300, 234],
        [138, 234],
    ],
    //二区
    A2208_A2204: [
        [300, 234],
        [340, 234],
    ],
    A2208_A2205: [
        [300, 234],
        [262, 234],
    ],
    A2208_A2202: [
        [300, 234],
        [447, 234],
        [442, 234],
        [442, 454],
        [447, 454],
        [336, 454],
    ],
    A2208_A2201: [
        [300, 234],
        [447, 234],
        [442, 234],
        [442, 454],
        [447, 454],
        [264, 454],
    ],
    A2208_A2212: [
        [300, 234],
        [38, 234],
        [38, 234],
        [38, 342],
    ],
    // //3区
    A2208_A2216: [
        [300, 234],
        [447, 234],
        [442, 234],
        [442, 580],
        [447, 580],
        [356, 580],
    ],
    A2208_A2215: [
        [300, 234],
        [447, 234],
        [442, 234],
        [442, 580],
        [447, 580],
        [260, 580],
    ],
    A2208_A2229: [
        [300, 234],
        [38, 234],
        [38, 234],
        [38, 640],
    ],
    A2208_A2219: [
        [300, 234],
        [447, 234],
        [442, 234],
        [442, 814],
        [447, 814],
        [334, 814],
    ],
    A2208_A2220: [
        [300, 234],
        [447, 234],
        [442, 234],
        [442, 814],
        [447, 814],
        [260, 814],
    ],
    A2208_A2221: [
        [300, 234],
        [38, 234],
        [38, 234],
        [38, 814],
        [38, 814],
        [108, 814],
    ],

    // //4区
    A2208_A2223: [
        [300, 234],
        [447, 234],
        [442, 234],
        [442, 814],
        [447, 814],
        [350, 814],
    ],
    A2208_A2222: [
        [300, 234],
        [447, 234],
        [442, 234],
        [442, 814],
        [447, 814],
        [246, 814],
    ],
    A2208_A2228: [
        [300, 234],
        [38, 234],
        [38, 234],
        [38, 870],
    ],
    A2208_A2225: [
        [300, 234],
        [447, 234],
        [442, 234],
        [442, 1020],
        [447, 1020],
        [344, 1020],
    ],
    A2208_A2226: [
        [300, 234],
        [447, 234],
        [442, 234],
        [442, 1020],
        [447, 1020],
        [276, 1020],
    ],
    A2208_A2227: [
        [300, 234],
        [447, 234],
        [442, 234],
        [442, 1020],
        [447, 1020],
        [196, 1020],
    ],
    }
    $('[data-roomname]').on('click', function (e) {
      if(props.form.imgShow==2){
         return
      }
      $('#div2').css('zoom', 1)

      // 清除放大缩小滚动条定时器
      clearTimeout(scrolltimer)

      // 当有点击时清除body中 路径展示定时器 标识属性
      // clearTimeout($("body").attr("timeout-scale"));

      if ($('body').attr('locked') == 1) {
        return
      }

      $('body').attr('locked', 1)

      $('[data-roomname]').css('background-color', 'rgba(52, 127, 122, 1)')

      $('.line').remove()
      $('.endimg').remove()
      $('.startimg').remove()
      document.getElementById('zuobiaodian').style.opacity = '1'

      clearTimeout($('body').attr('timeout'))

      var end = $(this).attr('data-roomname')
      console.log(end)
      start_end(props.roomName.replace('-','')+'_' + end, end)
    //   start_end('A2225_' + end, end)
    })
    function start_end(xy, end) {
      for (var i = 0; i < CoordPoint[xy].length; i = i + 2) {
        // console.log(CoordPoint[xy][i]);

        if (i + 2 <= CoordPoint[xy].length) {
          drawLine(CoordPoint[xy][i], CoordPoint[xy][i + 1], i)
        } else {
          //   end(CoordPoint[xy][i][0], CoordPoint[xy][i][1]);
        }
      }

      // var length = $(".line").length;
      if ($('.line').length == 1) {
        document.getElementById('zuobiaodian').style.opacity = '0'
        $('#div2').append(
          "<img class='startimg' src='http://39.105.179.38:9797/noderad/start1.png' style='position:absolute;width:38px;height:38px;left:" +
            (CoordPoint[xy][0][0] - 15) +
            'px;top:' +
            (CoordPoint[xy][0][1] - 15) +
            "px';>"
        )
        // 如果是1条线
        re($('.line').eq(0), function () {
          $('#div2').append(
            "<img class='endimg' src='http://39.105.179.38:9797/noderad/end1.png' style='position:absolute;width:38px;height:38px;left:" +
              (CoordPoint[xy][i - 1][0] - 15) +
              'px;top:' +
              (CoordPoint[xy][i - 1][1] - 15) +
              "px';>"
          )
          $('#' + end).css('background-color', 'rgba(255, 145, 0, 1)')

          $('body').removeAttr('locked')
          var timeout = setTimeout(function () {
            $('#' + end).css('background-color', 'rgba(52, 127, 122, 1)')
            $('.line').remove()
            $('.endimg').remove()
            $('.startimg').remove()
            document.getElementById('zuobiaodian').style.opacity = '1'

            //动态路径展示完毕，调用放大缩小全屏展示函数
            scrollSwiper()
          }, 15000)
          // 给body添加路径展示定时器 标识
          $('body').attr('timeout', timeout)

          // var scaletimeout = setTimeout(function () {
          //     $("#div2").css("zoom", 1.972);
          //     let ele = document.getElementById('div2cont');
          //     ele.scrollTop = 1080;
          // }, 10000);
          // $("body").attr("timeout-scale", scaletimeout);
        })
      } else if ($('.line').length == 2) {
        document.getElementById('zuobiaodian').style.opacity = '0'
        $('#div2').append(
          "<img class='startimg' src='http://39.105.179.38:9797/noderad/start1.png' style='position:absolute;width:38px;height:38px;left:" +
            (CoordPoint[xy][0][0] - 15) +
            'px;top:' +
            (CoordPoint[xy][0][1] - 15) +
            "px';>"
        )
        // 如果是2条线
        re($('.line').eq(0), function () {
          re($('.line').eq(1), function () {
            $('#div2').append(
              "<img class='endimg' src='http://39.105.179.38:9797/noderad/end1.png' style='position:absolute;width:38px;height:38px;left:" +
                (CoordPoint[xy][i - 1][0] - 15) +
                'px;top:' +
                (CoordPoint[xy][i - 1][1] - 15) +
                "px';>"
            )
            $('#' + end).css('background-color', 'rgba(255, 145, 0, 1)')

            $('body').removeAttr('locked')
            var timeout = setTimeout(function () {
              $('#' + end).css('background-color', 'rgba(52, 127, 122, 1)')
              $('.line').remove()
              $('.endimg').remove()
              $('.startimg').remove()
              document.getElementById('zuobiaodian').style.opacity = '1'

              //动态路径展示完毕，调用放大缩小全屏展示函数
              scrollSwiper()
            }, 15000)
            // 给body添加路径展示定时器 标识
            $('body').attr('timeout', timeout)

            // var scaletimeout = setTimeout(function () {
            //     $("#div2").css("zoom", 1.972);
            //     let ele = document.getElementById('div2cont');
            //     ele.scrollTop = 1080;
            // }, 10000);
            // $("body").attr("timeout-scale", scaletimeout);
          })
        })
      } else if ($('.line').length == 3) {
        document.getElementById('zuobiaodian').style.opacity = '0'
        $('#div2').append(
          "<img class='startimg' src='http://39.105.179.38:9797/noderad/start1.png' style='position:absolute;width:38px;height:38px;left:" +
            (CoordPoint[xy][0][0] - 15) +
            'px;top:' +
            (CoordPoint[xy][0][1] - 15) +
            "px';>"
        )
        // 如果是3条线
        re($('.line').eq(0), function () {
          re($('.line').eq(1), function () {
            re($('.line').eq(2), function () {
              $('#div2').append(
                "<img class='endimg' src='http://39.105.179.38:9797/noderad/end1.png' style='position:absolute;width:38px;height:38px;left:" +
                  (CoordPoint[xy][i - 1][0] - 18) +
                  'px;top:' +
                  (CoordPoint[xy][i - 1][1] - 15) +
                  "px';>"
              )
              $('#' + end).css('background-color', 'rgba(255, 145, 0, 1)')

              $('body').removeAttr('locked')
              var timeout = setTimeout(function () {
                $('#' + end).css('background-color', 'rgba(52, 127, 122, 1)')
                $('.line').remove()
                $('.endimg').remove()
                $('.startimg').remove()
                document.getElementById('zuobiaodian').style.opacity = '1'

                //动态路径展示完毕，调用放大缩小全屏展示函数
                scrollSwiper()
              }, 15000)
              // 给body添加路径展示定时器 标识
              $('body').attr('timeout', timeout)

              // var scaletimeout = setTimeout(function () {
              //     $("#div2").css("zoom", 1.972);
              //     let ele = document.getElementById('div2cont');
              //     ele.scrollTop = 1080;
              // }, 10000);
              // $("body").attr("timeout-scale", scaletimeout);
            })
          })
        })
      } else if ($('.line').length == 4) {
        document.getElementById('zuobiaodian').style.opacity = '0'
        $('#div2').append(
          "<img class='startimg' src='http://39.105.179.38:9797/noderad/start1.png' style='position:absolute;width:38px;height:38px;left:" +
            (CoordPoint[xy][0][0] - 15) +
            'px;top:' +
            (CoordPoint[xy][0][1] - 15) +
            "px';>"
        )
        // 如果是4条线
        re($('.line').eq(0), function () {
          re($('.line').eq(1), function () {
            re($('.line').eq(2), function () {
              re($('.line').eq(3), function () {
                //  debugger
                $('#div2').append(
                  "<img class='endimg' src='http://39.105.179.38:9797/noderad/end1.png' style='position:absolute;width:38px;height:38px;left:" +
                    (CoordPoint[xy][i - 1][0] - 15) +
                    'px;top:' +
                    (CoordPoint[xy][i - 1][1] - 15) +
                    "px';>"
                )
                $('#' + end).css('background-color', 'rgba(255, 145, 0, 1)')

                $('body').removeAttr('locked')
                var timeout = setTimeout(function () {
                  $('#' + end).css('background-color', 'rgba(52, 127, 122, 1)')
                  $('.line').remove()
                  $('.endimg').remove()
                  $('.startimg').remove()
                  document.getElementById('zuobiaodian').style.opacity = '1'

                  //动态路径展示完毕，调用放大缩小全屏展示函数
                  scrollSwiper()
                }, 15000)
                // 给body添加路径展示定时器 标识
                $('body').attr('timeout', timeout)

                // var scaletimeout = setTimeout(function () {
                //     $("#div2").css("zoom", 1.972);
                //     let ele = document.getElementById('div2cont');
                //     ele.scrollTop = 1080;
                // }, 10000);
                // $("body").attr("timeout-scale", scaletimeout);
              })
            })
          })
        })
      } else if ($('.line').length == 5) {
        document.getElementById('zuobiaodian').style.opacity = '0'
        $('#div2').append(
          "<img class='startimg' src='http://39.105.179.38:9797/noderad/start1.png' style='position:absolute;width:38px;height:38px;left:" +
            (CoordPoint[xy][0][0] - 15) +
            'px;top:' +
            (CoordPoint[xy][0][1] - 15) +
            "px';>"
        )
        // 如果是5条线
        re($('.line').eq(0), function () {
          re($('.line').eq(1), function () {
            re($('.line').eq(2), function () {
              re($('.line').eq(3), function () {
                re($('.line').eq(4), function () {
                  $('#div2').append(
                    "<img class'endimg' src='http://39.105.179.38:9797/noderad/end1.png' style='position:absolute;width:38px;height:38px;left:" +
                      (CoordPoint[xy][i - 1][0] - 15) +
                      'px;top:' +
                      (CoordPoint[xy][i - 1][1] - 15) +
                      "px';>"
                  )
                  $('#' + end).css('background-color', 'rgba(255, 145, 0, 1)')

                  $('body').removeAttr('locked')
                  var timeout = setTimeout(function () {
                    $('#' + end).css('background-color', 'rgba(52, 127, 122, 1)')
                    $('.line').remove()
                    $('.endimg').remove()
                    $('.startimg').remove()
                    document.getElementById('zuobiaodian').style.opacity = '1'

                    //动态路径展示完毕，调用放大缩小全屏展示函数
                    scrollSwiper()
                  }, 15000)
                  // 给body添加路径展示定时器 标识
                  $('body').attr('timeout', timeout)

                  // var scaletimeout = setTimeout(function () {
                  //     $("#div2").css("zoom", 1.972);
                  //     let ele = document.getElementById('div2cont');
                  //     ele.scrollTop = 1080;
                  // }, 10000);
                  // $("body").attr("timeout-scale", scaletimeout);
                })
              })
            })
          })
        })
      }

      function re($line, callback) {
        if ($line.attr('data-type') == 'x') {
          $line
            .find('div')
            .stop()
            .animate({ width: $line.width() }, 'fast', 'linear', function () {
              callback()
            })
        } else if ($line.attr('data-type') == 'y') {
          $line
            .find('div')
            .stop()
            .animate({ height: $line.height() }, 'fast', 'linear', function () {
              callback()
            })
        } else {
        }
      }
    }
    function drawLine(start, end) {
      var div = document.createElement('div')
      div.style.position = 'absolute'
      div.className = 'line'

      // div.style.backgroundColor = "red";
      //添加子div
      var children = document.createElement('div')
      children.style.backgroundColor = 'orange'

      div.appendChild(children)

      var div2 = document.getElementById('div2')
      div2.appendChild(div)

      // 如果线与Y轴平行
      if (start[0] == end[0]) {
        div.setAttribute('data-type', 'y')
        if (start[1] > end[1]) {
          // 反方向
          div.setAttribute('data-forword', 'minus')
          div.style.transform = 'rotate(180deg)'
        } else {
          // 同方向
          div.setAttribute('data-forword', 'add')
        }

        div.style.width = 5 + 'px'
        div.style.height = Math.abs(start[1] - end[1]) + 'px'
        children.style.height = 0
        children.style.width = 5 + 'px'

        // 判断起点 终点距离 分别与原点比较
        var abs1 = Math.sqrt(start[0] * start[0] + start[1] * start[1])
        var abs2 = Math.sqrt(end[0] * end[0] + end[1] * end[1])
        if (abs1 > abs2) {
          // debugger
          div.style.top = end[1] + 'px'
          div.style.left = end[0] + 'px'
        } else {
          div.style.top = start[1] + 'px'
          div.style.left = start[0] + 'px'
        }
      } else if (start[1] == end[1]) {
        div.setAttribute('data-type', 'x')
        if (start[0] > end[0]) {
          // 反方向
          div.setAttribute('data-forword', 'minus')
          div.style.transform = 'rotate(180deg)'
        } else {
          // 同方向
          div.setAttribute('data-forword', 'add')
        }
        //如果与 x轴平行
        div.style.width = Math.abs(start[0] - end[0]) + 'px'
        div.style.height = 5 + 'px'

        children.style.height = 5 + 'px'
        children.style.width = 0
        // 判断起点 终点距离 分别与原点比较
        var abs1 = Math.sqrt(start[0] * start[0] + start[1] * start[1])
        var abs2 = Math.sqrt(end[0] * end[0] + end[1] * end[1])
        if (abs1 > abs2) {
          div.style.top = end[1] + 'px'
          div.style.left = end[0] + 'px'
        } else {
          div.style.top = start[1] + 'px'
          div.style.left = start[0] + 'px'
        }
      } else {
      }
    }
  })
})
</script>

<style lang="less" scoped>
ul,li {
  list-style: none;
  margin: 0;
  padding: 0;
}
.div {
  width: 1920px;
  height: 1080px;
  background-color: #2c6965;
  /* border: 1px solid red; */
  /*zoom: 0.345545454;*/
  zoom: 0.354245454;
  display: flex;
  justify-content: space-between;
}

/* 右侧区域 */
.div1 {
  width: 21.09%;
  height: 100%;
  /*margin-left: 90px;*/
  margin-top: 16px;
}

.div1 ul {
  margin-left: 28px;
  margin-top: 262px;
}

.div1 ul li {
  margin-bottom: 40px;
  height: 44px;
  display: flex;
  align-items: center;
}

.div1 ul li img {
  width: 44px;
  height: 44px;
  vertical-align: top;
  margin: 0;
  margin-left: 17px;
  margin-right: 28px;
}

.div1 ul li:nth-child(1) img {
  width: 56px;
  height: 56px;
  margin-left: 10px;
  background-color: transparent;
}

.div1 ul li:nth-child(4) img {
  background-color: rgba(52, 127, 122, 1);
}

.div1 ul li span {
  display: inline-block;
  height: 44px;
  line-height: 44px;
  white-space: nowrap;
  color: rgba(255, 255, 255, 0.8);
  font-size: 30px;
  text-align: center;
  font-family: SourceHanSansSC-regular;
}

.div1 ul li:last-child span:nth-child(1) {
  width: 44px;
  height: 44px;
  background-color: rgba(255, 145, 0, 1);
  margin: 0 24px 0 16px;
}

.div1 ul li:last-child span:nth-child(2) {
  /*transform: translateY(-12px);*/
}

/* 添加滚动条 */
::-webkit-scrollbar {
  width: 6px;
  height: 6px;
}

::-webkit-scrollbar-thumb {
  height: 50px;
  /* background-color: #999; */
  background-color: #2c6965;
  -webkit-border-radius: 4px;
  /* outline: 2px solid #ededed; */
  outline: 2px solid #2c6965;
  outline-offset: -2px;
}

/* 中间区域 */
.div2cont {
  height: 1080px;
  overflow-x: hidden;
  overflow-y: scroll;
}

.div2 {
  height: 1042px;
  /* margin-top: 16px;
            margin-bottom: 22px; */
  background-color: rgba(255, 255, 255, 0.04);
  position: relative;
  display: flex;
  justify-content: space-between;
  zoom: 1.972;
}

/* 中间中部 */
.div2 img {
  margin: 0;
  vertical-align: top;
  background-color: transparent;
}

/* 中间左侧 */
.div2 .div2-1,
.div2 .div2-3 {
  width: 60px;
  height: 1042px;
  line-height: 136px;
  color: rgba(255, 255, 255, 0.2);
  font-size: 22px;
  text-align: center;
  font-family: Roboto;
}

/* 中间中部 */
.div2 .div2-2 {
  width: 366px;
  height: 1080px;
  /* border: 1px solid red; */
  /* 所有会议室公用 */
  color: rgba(255, 255, 255, 1);
  font-size: 22px;
  text-align: center;
  font-family: Roboto;
  background-color: #2c6965;
}

.div2 .div2-2 .left .left1,
.div2 .div2-2 .right .right5 {
  width: 366px;
  height: 34px;
  line-height: 34px;
  background-color: rgba(255, 255, 255, 0.04);
  color: rgba(255, 255, 255, 0.2);
  font-size: 18px;
  text-align: center;
  font-family: Roboto;
}

.div2 .div2-2 .left .left3,
.div2 .div2-2 .right .right3 {
  width: 100%;
  height: 31px;
  line-height: 31px;
  background-color: rgba(255, 255, 255, 0.04);
  color: rgba(255, 255, 255, 0.2);
  font-size: 18px;
  text-align: center;
  font-family: Roboto;
  margin-top: 6px;
  margin-bottom: 6px;
}

.div2 .div2-2 .left .left5,
.div2 .div2-2 .right .right1 {
  width: 366px;
  height: 58px;
  line-height: 58px;
  display: flex;
  justify-content: space-between;
}

.div2 .div2-2 .left .left2 {
  /*height: 176px;*/
  height: 182px;
  display: flex;
  justify-content: space-between;
  /* border: 1px solid red; */
  padding: 6px 6px 0px 6px;
}

.div2 .div2-2 .left .left2 .left2-1 {
  width: 100px;
}

.div2 .div2-2 .left .left2 .left2-1 div:nth-child(1),
.div2 .div2-2 .left .left2 .left2-1 div:nth-child(2) {
  position: relative;
}

.div2 .div2-2 .left .left2 .left2-1 div:nth-child(1) > div {
  width: 100px;
  height: 120px;
  line-height: 120px;
  box-shadow: 3px 2px 1px 0px rgba(0, 0, 0, 0.4);
  background-color: rgba(52, 127, 122, 1);
  margin-bottom: 6px;
}

.div2 .div2-2 .left .left2 .left2-1 div:nth-child(1) img:nth-child(2) {
  width: 6px;
  height: 42px;
  border-radius: 1px;
  box-shadow: 0px 1px 1px 0px rgba(0, 0, 0, 0.4);
  position: absolute;
  top: 8px;
  left: 0;
}

.div2 .div2-2 .left .left2 .left2-1 div:nth-child(1) img:nth-child(3) {
  width: 6px;
  height: 42px;
  border-radius: 1px;
  box-shadow: 0px 1px 1px 0px rgba(0, 0, 0, 0.4);
  position: absolute;
  bottom: 8px;
  left: 0;
}

.div2 .div2-2 .left .left2 .left2-1 div:nth-child(2) > div {
  width: 100px;
  height: 50px;
  font-size: 18px;
  line-height: 21px;
  box-shadow: 3px 2px 1px 0px rgba(0, 0, 0, 0.4);
  background-color: rgba(52, 127, 122, 1);
  margin-bottom: 4px;
}

.div2 .div2-2 .left .left2 .left2-1 div:nth-child(2) img {
  width: 20px;
  height: 6px;
  border-radius: 1px;
  box-shadow: 0px 1px 1px 0px rgba(0, 0, 0, 0.4);
  position: absolute;
  bottom: 0px;
  right: 18px;
}

.div2 .div2-2 .left .left2 .left2-2 {
  width: 204px;
}

.div2 .div2-2 .left .left2 .left2-2 > div:nth-child(1) {
  width: 100%;
  height: 40px;
  margin-bottom: 4px;
  border-radius: 4px;
  background-color: rgba(65, 116, 116, 1);
  box-shadow: 3px 2px 1px 0px rgba(0, 0, 0, 0.4);
}

.div2 .div2-2 .left .left2 .left2-2 div:nth-child(2) {
  width: 100%;
  height: 132px;
  display: flex;
  justify-content: space-between;
}

.div2 .div2-2 .left .left2 .left2-2 > div:nth-child(2) > div {
  position: relative;
}

.div2 .div2-2 .left .left2 .left2-2 > div:nth-child(2) > div > div {
  width: 100px;
  height: 130px;
  line-height: 130px;
  background-color: rgba(52, 127, 122, 1);
  box-shadow: 3px 2px 1px 0px rgba(0, 0, 0, 0.4);
  margin-bottom: 4px;
  margin-right: 4px;
}

.door {
  width: 20px;
  height: 5px;
  opacity: 0.7;
}

.div2 .div2-2 .left .left2 .left2-2 div:nth-child(2) > div img:nth-child(2) {
  position: absolute;
  bottom: 0px;
  left: 18px;
}

.div2 .div2-2 .left .left2 .left2-2 div:nth-child(2) > div img:nth-child(3) {
  position: absolute;
  bottom: 0px;
  right: 18px;
}

.div2 .div2-2 .left .left2 .left2-3 {
  width: 40px;
}

.div2 .div2-2 .left .left2 .left2-3 > div:nth-child(1) {
  width: 40px;
  height: 52px;
  border-radius: 4px;
  margin-bottom: 4px;
  background-color: rgba(65, 116, 116, 1);
  box-shadow: 3px 2px 1px 0px rgba(0, 0, 0, 0.4);
}

.div2 .div2-2 .left .left2 .left2-3 > img {
  width: 40px;
  height: 78px;
  box-shadow: 3px 2px 1px 0px rgba(0, 0, 0, 0.4);
}

.div2 .div2-2 .left .left2 .left2-3 > div:nth-child(3) {
  /* width: 34px;
    height: 34px; */
}

.div2 .div2-2 .left .left2 .left2-3 > div:nth-child(3) img {
  width: 34px;
  height: 34px;
  margin: 4px;
}

/*会议室2区 */
.div2 .div2-2 .left .left4 {
  display: flex;
  justify-content: space-between;
  margin-bottom: 4px;
}

.div2 .div2-2 .left .left4 .left4-1 > div,
.div2 .div2-2 .left .left4 .left4-2 > div,
.div2 .div2-2 .left .left4 .left4-3 > div {
  position: relative;
}

.div2 .div2-2 .left .left4 .left4-1 {
  width: 100px;
  height: 177px;
  margin-left: 6px;
}

.div2 .div2-2 .left .left4 .left4-1 > div:nth-child(1) {
  width: 100%;
  height: 120px;
  line-height: 120px;
  margin-bottom: 4px;
  border-radius: 4px;
  background-color: rgba(52, 127, 122, 1);
  box-shadow: 3px 2px 1px 0px rgba(0, 0, 0, 0.4);
}

.hdoubledoor {
  width: 6px;
  height: 42px;
}

.doubledoor {
  width: 42px;
  height: 6px;
}

.div2 .div2-2 .left .left4 .left4-1 > div:nth-child(1) img:nth-child(2) {
  position: absolute;
  left: 0;
  top: 7px;
}

.div2 .div2-2 .left .left4 .left4-1 > div:nth-child(1) img:nth-child(3) {
  position: absolute;
  left: 0;
  bottom: 7px;
}

.div2 .div2-2 .left .left4 .left4-1 > div:nth-child(2) {
  width: 100px;
  height: 53px;
  line-height: 51px;
  border-radius: 4px;
  background-color: rgba(65, 116, 116, 1);
  box-shadow: 3px 2px 1px 0px rgba(0, 0, 0, 0.4);
}

.div2 .div2-2 .left .left4 .left4-2 {
  width: 120px;
  height: 177px;
}

.div2 .div2-2 .left .left4 .left4-2 > div > div {
  width: 120px;
  height: 85px;
  margin-bottom: 4px;
  line-height: 77px;
  border-radius: 4px;
  background-color: rgba(52, 127, 122, 1);
  box-shadow: 3px 2px 1px 0px rgba(0, 0, 0, 0.4);
}

.div2 .div2-2 .left .left4 .left4-2 div:nth-child(1) img:nth-child(2) {
  position: absolute;
  top: 0px;
  left: 10px;
}

.div2 .div2-2 .left .left4 .left4-2 div:nth-child(1) img:nth-child(3) {
  position: absolute;
  top: 0px;
  right: 10px;
}

.div2 .div2-2 .left .left4 .left4-2 div:nth-child(2) img:nth-child(2) {
  position: absolute;
  bottom: 0px;
  left: 10px;
}

.div2 .div2-2 .left .left4 .left4-2 div:nth-child(2) img:nth-child(3) {
  position: absolute;
  bottom: 0px;
  right: 10px;
}

.div2 .div2-2 .left .left4 .left4-3 {
  width: 80px;
  height: 177px;
}

.div2 .div2-2 .left .left4 .left4-3 > div > div {
  width: 80px;
  height: 85px;
  margin-bottom: 4px;
  line-height: 85px;
  font-size: 20px;
  border-radius: 4px;
  background-color: rgba(52, 127, 122, 1);
  box-shadow: 3px 2px 1px 0px rgba(0, 0, 0, 0.4);
}

.div2 .div2-2 .left .left4 .left4-3 > div:nth-child(2) > div {
  height: 85px;
  line-height: 21px;
  font-size: 20px;
}

.div2 .div2-2 .left .left4 .left4-3 div:nth-child(1) img:nth-child(2) {
  position: absolute;
  top: 0px;
  left: 18px;
}

.div2 .div2-2 .left .left4 .left4-3 div:nth-child(1) img:nth-child(3) {
  position: absolute;
  top: 0px;
  right: 18px;
}

.div2 .div2-2 .left .left4 .left4-3 div:nth-child(2) img:nth-child(2) {
  position: absolute;
  bottom: 0px;
  left: 18px;
}

.div2 .div2-2 .left .left4 .left4-3 div:nth-child(2) img:nth-child(3) {
  position: absolute;
  bottom: 0px;
  right: 18px;
}

.div2 .div2-2 .left .left4 .left4-4 {
  width: 40px;
  height: 177px;
  margin-right: 6px;
}

.div2 .div2-2 .left .left4 .left4-4 div:nth-child(1) {
  width: 40px;
  height: 85px;
  margin-bottom: 4px;
  border-radius: 4px;
  background-color: rgba(65, 116, 116, 1);
  box-shadow: 3px 2px 1px 0px rgba(0, 0, 0, 0.4);
}

.div2 .div2-2 .left .left4 .left4-4 img {
  width: 40px;
  height: 85px;
  box-shadow: 3px 2px 1px 0px rgba(0, 0, 0, 0.4);
}

.div2 .div2-2 .left .left5 > div:nth-child(1) > div:nth-child(1),
.div2 .div2-2 .right .right1 > div:nth-child(1) > div:nth-child(2) {
  width: 100px;
  height: 20px;
  margin-left: 6px;
  background-color: #2c6965;
  display: flex;
  justify-content: flex-start;
}

.div2 .div2-2 .right .right1 > div:nth-child(1) > div:nth-child(1) {
  margin-top: 30px;
}

.div2 .div2-2 .left .left5 > div:nth-child(1) > div:nth-child(1) > div,
.div2 .div2-2 .right .right1 > div:nth-child(1) > div:nth-child(2) > div {
  width: 48px;
  height: 20px;
  margin-top: 4px;
  margin-right: 4px;
  border-radius: 4px;
  background-color: rgba(65, 116, 116, 1);
  box-shadow: 3px 2px 1px 0px rgba(0, 0, 0, 0.4);
}

.div2 .div2-2 .left .left5 > div:nth-child(1) > div:nth-child(1) > div img,
.div2 .div2-2 .right .right1 > div:nth-child(1) > div:nth-child(2) > div img {
  width: 18px;
  height: 18px;
  margin: 1px 15px;
}

.div2 .div2-2 .left .left5 > div:nth-child(1) > div:nth-child(2),
.div2 .div2-2 .right .right1 > div:nth-child(1) > div:nth-child(1) {
  width: 112px;
  height: 31px;
  margin-top: 10px;
  background-color: rgba(255, 255, 255, 0.04);
}

.div2 .div2-2 .right .right1 > div:nth-child(1) > div:nth-child(1) {
  margin-top: 0px;
}

.div2 .div2-2 .left .left5 > div:nth-child(2),
.div2 .div2-2 .right .right1 > div:nth-child(2) {
  width: 254px;
  height: 58px;
  line-height: 58px;
  background-color: rgba(255, 255, 255, 0.04);
  color: rgba(255, 255, 255, 0.2);
  font-size: 18px;
  text-align: center;
  font-family: Roboto;
}

/*中间楼梯 */
.div2 .div2-2 .middle {
  width: 100%;
  height: 48px;
  /* border:1px solid red; */
  display: flex;
  justify-content: space-between;
  background-color: rgba(255, 255, 255, 0.04);
}

.div2 .div2-2 .middle > div:nth-child(1) {
  display: flex;
  justify-content: flex-start;
  /* width: 100px;
    height: 35px; */
  background-color: #2c6965;
}

.div2 .div2-2 .middle > div:nth-child(1) > div {
  width: 48px;
  height: 35px;
  text-align: center;
  border-radius: 4px;
  margin: 6px;
  margin-right: 4px;
  background-color: rgba(65, 116, 116, 1);
  box-shadow: 3px 2px 1px 0px rgba(0, 0, 0, 0.4);
  display: flex;
  justify-content: space-evenly;
}

.div2 .div2-2 .middle > div:nth-child(1) > div img {
  width: 15px;
  height: 15px;
  margin-top: 10px;
}

.div2 .div2-2 .middle > div:nth-child(1) > img:nth-child(2) {
  width: 24px;
  height: 35px;
  margin-top: 6px;
  margin-right: 0px;
  background-color: rgba(65, 116, 116, 1);
  box-shadow: 3px 2px 1px 0px rgba(0, 0, 0, 0.4);
}

.div2 .div2-2 .middle > div:nth-child(1) > img:nth-child(3) {
  width: 24px;
  height: 35px;
  margin-top: 6px;
  margin-right: 6px;
  background-color: rgba(65, 116, 116, 1);
  box-shadow: 3px 2px 1px 0px rgba(0, 0, 0, 0.4);
}

.div2 .div2-2 .middle > div:nth-child(2) {
  width: 150px;
  height: 48px;
  background-color: #2c6965;
}

.div2 .div2-2 .middle > div:nth-child(2) div {
  width: 150px;
  margin: 5px 0;
}

.div2 .div2-2 .middle > div:nth-child(2) > div div {
  height: 38px;
  background-color: rgba(255, 255, 255, 0.04);
}

.div2 .div2-2 .middle > div:nth-child(2) > div div img {
  width: 38px;
  height: 38px;
  border-radius: 50%;
}

/* 中间右侧 */
/*会议室3区 */
.div2 .div2-2 .right .right2 {
  display: flex;
  justify-content: space-between;
  height: 186px;
  margin-bottom: 4px;
  margin-top: 6px;
  color: rgba(255, 255, 255, 1);
  font-size: 22px;
}

.div2 .div2-2 .right .right2 .right2-1 > div,
.div2 .div2-2 .right .right2 .right2-2 > div,
.div2 .div2-2 .right .right2 .right2-3 > div {
  position: relative;
}

.div2 .div2-2 .right .right2 .right2-1 {
  width: 100px;
  height: 100%;
  margin-left: 6px;
}

.div2 .div2-2 .right .right2 .right2-1 > div:nth-child(1) {
  width: 100px;
  height: 110px;
  line-height: 110px;
  margin-bottom: 4px;
  border-radius: 4px;
  background-color: rgba(52, 127, 122, 1);
  box-shadow: 3px 2px 1px 0px rgba(0, 0, 0, 0.4);
}

.div2 .div2-2 .right .right2 .right2-1 > div:nth-child(1) img:nth-child(2) {
  position: absolute;
  left: 0;
  top: 10px;
}

.div2 .div2-2 .right .right2 .right2-1 > div:nth-child(1) img:nth-child(3) {
  position: absolute;
  left: 0;
  bottom: 10px;
}

.div2 .div2-2 .right .right2 .right2-1 > div:nth-child(2) {
  width: 100px;
  height: 70px;
  line-height: 70px;
  border-radius: 4px;
  background-color: rgba(52, 127, 122, 1);
  box-shadow: 3px 2px 1px 0px rgba(0, 0, 0, 0.4);
}

.div2 .div2-2 .right .right2 .right2-1 > div:nth-child(2) img {
  position: absolute;
  bottom: 0;
  left: 18px;
}

.div2 .div2-2 .right .right2 .right2-2 {
  width: 120px;
  height: 100%;
}

.div2 .div2-2 .right .right2 .right2-2 > div > div {
  width: 120px;
  height: 90px;
  margin-bottom: 4px;
  line-height: 90px;
  border-radius: 4px;
  background-color: rgba(52, 127, 122, 1);
  box-shadow: 3px 2px 1px 0px rgba(0, 0, 0, 0.4);
}

.div2 .div2-2 .right .right2 .right2-2 div:nth-child(1) img:nth-child(2) {
  position: absolute;
  top: 0px;
  left: 10px;
}

.div2 .div2-2 .right .right2 .right2-2 div:nth-child(1) img:nth-child(3) {
  position: absolute;
  top: 0px;
  right: 10px;
}

.div2 .div2-2 .right .right2 .right2-2 div:nth-child(2) img:nth-child(2) {
  position: absolute;
  bottom: 0px;
  left: 10px;
}

.div2 .div2-2 .right .right2 .right2-2 div:nth-child(2) img:nth-child(3) {
  position: absolute;
  bottom: 0px;
  right: 10px;
}

.div2 .div2-2 .right .right2 .right2-3 {
  width: 80px;
  height: 100%;
}

.div2 .div2-2 .right .right2 .right2-3 > div > div {
  width: 80px;
  height: 90px;
  margin-bottom: 4px;
  line-height: 90px;
  border-radius: 4px;
  background-color: rgba(52, 127, 122, 1);
  box-shadow: 3px 2px 1px 0px rgba(0, 0, 0, 0.4);
}

.div2 .div2-2 .right .right2 .right2-3 > div:nth-child(1) > div {
  height: 90px;
  line-height: 21px;
  font-size: 20px;
}

.div2 .div2-2 .right .right2 .right2-3 div:nth-child(1) img:nth-child(2) {
  position: absolute;
  top: 0px;
  left: 10px;
}

.div2 .div2-2 .right .right2 .right2-3 div:nth-child(1) img:nth-child(3) {
  position: absolute;
  top: 0px;
  right: 10px;
}

.div2 .div2-2 .right .right2 .right2-3 div:nth-child(2) img:nth-child(2) {
  position: absolute;
  bottom: 0px;
  left: 18px;
}

.div2 .div2-2 .right .right2 .right2-3 div:nth-child(2) img:nth-child(3) {
  position: absolute;
  bottom: 0px;
  right: 18px;
}

.div2 .div2-2 .right .right2 .right2-4 {
  width: 42px;
  height: 100%;
  margin-right: 6px;
}

.div2 .div2-2 .right .right2 .right2-4 div:nth-child(2) {
  width: 42px;
  height: 90px;
  margin-top: 4px;
  margin-bottom: 4px;
  border-radius: 4px;
  background-color: rgba(65, 116, 116, 1);
  box-shadow: 3px 2px 1px 0px rgba(0, 0, 0, 0.4);
}

.div2 .div2-2 .right .right2 .right2-4 img {
  width: 42px;
  height: 90px;
  box-shadow: 3px 2px 1px 0px rgba(0, 0, 0, 0.4);
}

/*会议室4区 */
.div2 .div2-2 .right .right4 {
  height: 166px;
  margin-bottom: 4px;
  margin-top: 6px;
  display: flex;
  justify-content: space-between;
  position: relative;
}

.div2 .div2-2 .right .right4 .right4-1 > div,
.div2 .div2-2 .right .right4 .right4-2 > div,
.div2 .div2-2 .right .right4 .right4-3 > div {
  position: relative;
}

.div2 .div2-2 .right .right4 .right4-1 {
  width: 100px;
  /* height: 172px; */
  margin-right: 4px;
  margin-left: 6px;
}

.div2 .div2-2 .right .right4 .right4-1 > div:nth-child(1) > div {
  width: 100px;
  height: 120px;
  line-height: 120px;
  margin-bottom: 4px;
  border-radius: 4px;
  background-color: rgba(52, 127, 122, 1);
  box-shadow: 3px 2px 1px 0px rgba(0, 0, 0, 0.4);
}

.div2 .div2-2 .right .right4 .right4-1 > div:nth-child(1) img:nth-child(2) {
  position: absolute;
  left: 0;
  top: 10px;
}

.div2 .div2-2 .right .right4 .right4-1 > div:nth-child(1) img:nth-child(3) {
  position: absolute;
  left: 0;
  bottom: 7px;
}

.div2 .div2-2 .right .right4 .right4-1 > div:nth-child(2) {
  width: 108px;
  height: 44px;
  background-color: rgba(255, 255, 255, 0.04);
  position: absolute;
  left: 0;
  bottom: -4px;
}

.div2 .div2-2 .right .right4 .right4-1 > div:nth-child(2) div {
  width: 59px;
  height: 29px;
  margin-left: 49px;
  background-color: #2c6965;
}

.div2 .div2-2 .right .right4 .right4-1 > div:nth-child(2) div img {
  width: 14px;
  height: 14px;
  margin-top: 6px;
}

.div2 .div2-2 .right .right4 .right4-2 {
  width: 204px;
  height: 100%;
  margin-right: 4px;
  margin-bottom: 6px;
  background-color: rgba(255, 255, 255, 0.04);
}

.div2 .div2-2 .right .right4 .right4-2 > div:nth-child(1) {
  width: 100%;
  height: 80px;
  margin-bottom: 4px;
  /* border: 1px solid red; */
  display: flex;
  justify-content: flex-start;
}

.div2 .div2-2 .right .right4 .right4-2 > div:nth-child(1) > div > div {
  width: 100px;
  height: 80px;
  line-height: 80px;
  margin-right: 4px;
  border-radius: 4px;
  background-color: rgba(52, 127, 122, 1);
  box-shadow: 3px 2px 1px 0px rgba(0, 0, 0, 0.4);
  position: relative;
}

.div2 .div2-2 .right .right4 .right4-2 > div:nth-child(1) img {
  position: absolute;
  top: 0;
  /* right:18px; */
}

.div2 .div2-2 .right .right4 .right4-2 > div:nth-child(2) {
  width: 100%;
  height: 80px;
  /* border: 1px solid red; */
  display: flex;
  justify-content: flex-start;
}

.div2 .div2-2 .right .right4 .right4-2 > div:nth-child(2) > div {
  width: 66px;
  height: 80px;
  line-height: 80px;
  margin-right: 4px;
  border-radius: 4px;
  background-color: rgba(52, 127, 122, 1);
  color: rgba(255, 255, 255, 1);
  font-size: 17px;
  text-align: center;
  box-shadow: 3px 2px 1px 0px rgba(0, 0, 0, 0.4);
  font-family: Roboto;
  position: relative;
}

.div2 .div2-2 .right .right4 .right4-2 > div:nth-child(2) > div:nth-child(3) {
  font-size: 17px;
}

.div2 .div2-2 .right .right4 .right4-2 > div:nth-child(2) > div:nth-child(3) {
  width: 64px;
}

.div2 .div2-2 .right .right4 .right4-2 > div:nth-child(2) > div:nth-child(1) img {
  position: absolute;
  bottom: 0px;
  left: 6px;
}

.div2 .div2-2 .right .right4 .right4-2 > div:nth-child(2) > div img {
  position: absolute;
  bottom: 0px;
  right: 2px;
}

.div2 .div2-2 .right .right4 .right4-3 {
  width: 42px;
  height: 100%;
  margin-right: 6px;
}

.div2 .div2-2 .right .right4 .right4-3 div:nth-child(1) {
  width: 42px;
  height: 80px;
  margin-bottom: 4px;
  border-radius: 4px;
  background-color: rgba(65, 116, 116, 1);
  box-shadow: 3px 2px 1px 0px rgba(0, 0, 0, 0.4);
}

.div2 .div2-2 .right .right4 .right4-3 img {
  width: 42px;
  height: 80px;
  box-shadow: 3px 2px 1px 0px rgba(0, 0, 0, 0.4);
}

.div2 .div2-2 .right .right5 {
  display: flex;
  justify-content: space-evenly;
  /* height: 38px; */
}

/* 左右指示 */
.div2 .arrowPoint {
  display: flex;
  justify-content: space-between;
}

.div2 .arrowPoint span {
  height: 41px;
  color: rgba(255, 255, 255, 0.8);
  font-size: 16px;
  text-align: left;
  font-family: SourceHanSansSC-regular;
}

.div2 .arrowPoint > img {
  width: 25px;
  height: 25px;
  /* margin-top: 8px; */
  margin-right: 10px;
  margin-left: 2px;
}

.div2 .arrowPoint > img:nth-child(4) {
  margin-right: 2px;
  margin-left: 10px;
}

.div2 .arrowPoint > img:nth-child(3) {
  width: 11px;
  height: 32px;
  margin: 0;
  margin-top: -4px;
}

/* 右侧区域 */
.div3 {
  width: 21.09%;
  height: 100%;
}

.div3 > img {
  width: 185px;
  /* height: 172px; */
  margin-top: 73px;
  margin-left: 105px;
  margin-bottom: 427px;
  background-color: transparent;
}

/* 提示语 */
.div3 > div:nth-child(2) img {
  width: 44px;
  height: 44px;
  vertical-align: middle;
  background-color: transparent;
}

.div3 > div:nth-child(2) span {
  height: 50px;
  color: rgba(255, 255, 255, 1);
  font-size: 34px;
  text-align: left;
  font-family: SourceHanSansSC-regular;
}

.div3 > div:nth-child(2) > div {
  /* width: 448px; */
  /* height: 50px; */
  line-height: 50px;
  color: rgba(255, 255, 255, 1);
  font-size: 34px;
  text-align: left;
  font-family: SourceHanSansSC-regular;
  margin-left: 40px;
}

/*左右指示*/
#zuobiao {
  height: 34px;
  position: absolute;
  left: 289px;
  top: 1014px;
}

// 起点的激活样式 A2-225/226/227通用
.active-start {
  width: 64px !important;
  height: 80px !important;
  /*line-height: 106px !important;*/
  line-height: 80px !important;
  background-color: rgba(255, 145, 0, 1) !important;
  font-size: 17px;
  /*background-image: url(http://39.105.179.38:9797/noderad/dangqian.png);
            background-repeat: no-repeat;
            background-position: center 14%;
            background-size: 38px 38px;*/
}
.active-startA2219 {
 width: 80px !important;
            height: 90px !important;
            line-height: 90px !important;
            background-color: rgba(255, 145, 0, 1) !important;
            font-size: 17px;
}
.active-startA2220 {
    width: 120px !important;
            height: 90px !important;
            line-height: 90px !important;
            background-color: rgba(255, 145, 0, 1) !important;
            font-size: 17px;
}
.active-startA2221 {
   width: 100px !important;
            height: 70px !important;
            line-height: 70px !important;
            background-color: rgba(255, 145, 0, 1) !important;
            font-size: 17px;
}
.active-startA2202 {
    width: 80px !important;
    height: 85px !important;
    line-height: 21px !important;
    background-color: rgba(255, 145, 0, 1) !important;
    font-size: 20px;
}
.active-startA2201 {
   width: 120px !important;
            height: 85px !important;
            line-height: 85px !important;
            background-color: rgba(255, 145, 0, 1) !important;
            font-size: 18px;
}
.active-startA2208 {
   width: 100px !important;
            height: 100px !important;
            line-height: 160px !important;
            background-color: rgba(255, 145, 0, 1) !important;
            font-size: 20px;
}
.active-startA2207 {
    width: 100px !important;
    height: 130px !important;
    line-height: 130px !important;
    background-color: rgba(255, 145, 0, 1) !important;
    font-size: 20px;
}
.active-startA2206 {
  width: 100px !important;
  height: 50px !important;
  line-height: 21px !important;
  background-color: rgba(255, 145, 0, 1) !important;
  font-size: 18px;
}


</style>
