...
const uploadByPieces = async ({ fileName, appId, storeLocation, file }) => {
  try {
    ...
    const startData = await axios({
      url: '/dfs-ui/dfs/v2/file/upload/chunk/start',
      method: 'post',
      data: {
        fileName,
        fileSize: file.size,
    ...
    const uploadChunk = (data) => {
      return new Promise((resolve, reject) => {
        axios({
          url: '/dfs-ui/dfs/v2/file/upload/chunk/upload',
          method: 'post',
          data,
          headers: {
            'Content-Type': 'multipart/form-data',
    ...
    if (indexList.length !== chunkCount) {
      ...
      // 分片上传文件完成接口
      await axios({
        url: '/dfs-ui/dfs/v2/file/upload/chunk/finish',
        method: 'post',
        data: {
          fileName,
          fileId,
