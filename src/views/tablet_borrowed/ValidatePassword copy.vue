<template>
  <div class="device">
    ...
    <div class="top">
      <h1>设备管理</h1>
      <el-button @click="router.push('/tablet')">返回</el-button>
    </div>
    <!-- 2.查询 -->
    <div class="searchInfo">
      <el-form :model="form">
        ...
        <el-form-item label="设备ip地址">
          <el-input v-model="form.deviceId" placeholder="请输入设备ip地址" />
        </el-form-item>
        ...
        <el-form-item label="区域">
          <el-select v-model="form.region" placeholder="全部" >
            ...
            <el-option label="A2-225会议室" value="A2-225会议室"/>
            <el-option label="A2-226会议室" value="A2-226会议室"/>
            <el-option label="A2-227会议室" value="A2-227会议室"/>
          </el-select>
        </el-form-item>
      </el-form>
    ...
    <div class="deviceList">
      ...
      <div>
        ...
        <el-dialog v-model="newdialogFormVisible" title="新增">
          <el-form :model="newForm" ref="newFormRef" :rules="newFormRules" >
            <el-form-item label="设备ID" :label-width="formLabelWidth" prop="deviceId">
              <el-input v-model="newForm.deviceId" autocomplete="off" />
            </el-form-item>
            <el-form-item label="品牌" :label-width="formLabelWidth" prop="brand">
              <el-input v-model="newForm.brand" autocomplete="off" />
            </el-form-item>
            <el-form-item label="设备名称" :label-width="formLabelWidth" prop="deviceName">
              <el-input v-model="newForm.deviceName" autocomplete="off" />
            </el-form-item>
            <el-form-item label="型号" :label-width="formLabelWidth" prop="modelNumber">
              <el-input v-model="newForm.modelNumber" autocomplete="off" />
            </el-form-item>
            <el-form-item label="设备IP地址" :label-width="formLabelWidth" prop="deviceIpAddress">
              <el-input v-model="newForm.deviceIpAddress" autocomplete="off" />
            </el-form-item>
            <el-form-item label="会议室" :label-width="formLabelWidth" prop="region">
              <el-select v-model="newForm.region" placeholder="请输入会议室名称" style="width:100%">
                <!-- multiple -->
                <el-option label="A2-225会议室" value="A2-225会议室" />
                <el-option label="A2-226会议室" value="A2-226会议室" />
                <el-option label="A2-227会议室" value="A2-227会议室" />
            ...
            <el-form-item label="端口号" :label-width="formLabelWidth" prop="port">
              <el-input v-model="newForm.port" autocomplete="off" />
            </el-form-item>
            <el-form-item label="状态" :label-width="formLabelWidth" prop="status">
              <el-input v-model="newForm.status" autocomplete="off" />
            </el-form-item>
          </el-form>
          <template #footer>
        ...
        <!-- 删除 -->
        <el-button @click="deletebtn()" class="deletebtn" :disabled="selectList.length==0">删除</el-button>
      </div>
      <!-- 3.2 设备列表-->
      <el-table
        :data="tableData"
      ...
      >
        ...
        <el-table-column prop="modelNumber" label="型号" width="220" />
    ...
    <div class="pagination-block">
      ...
      <img src="@/assets/index/deviceList/refresh.png" />
    </div>
  </div>
</template>
<script setup>
...
const deletebtn=()=>{
    ...
    ElMessageBox.confirm(
      ...
      .then(() => {
        ...
        //批量删除请求（成功后发查询请求）
        axios.delete("http://10.31.0.101:1880/device/delete?id="+id.join(",")).then(response=>{
...
const addItem=()=>{
  newFormRef.value.validate((valid) => {
    if (valid) {
      ...
      axios.post("http://10.31.0.101:1880/device/create",{
        data:{
