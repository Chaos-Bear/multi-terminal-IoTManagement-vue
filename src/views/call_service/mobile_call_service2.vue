<template>
  <div>
    ...
    <div style="">
      ...
      <el-button disabled type="primary" style="float: right" @click="openMeetingChat"
        >手动更新会议</el-button
      >
    </div>
        ...
        <div class="call_service_msg_area">
            ...
            <template v-for="obj2 in chatMessages.data">
              <template v-if="obj2.name !== userInfo.userName">
                <div class="call_service_chat_messages">
                  <div class="call_service_chat" style="border-radius: 0 20px 20px 20px">
                    <span> {{ obj2.msg }} </span>
                  </div>
                </div>
              </template>
              <template v-else>
                <div
                  class="call_service_chat_messages"
          ...
          <template v-else>
            <div style="text-align: center">
              <span style="color: rgba(255, 255, 255, 0.4); width: 290px"> 暂无记录 </span>
            </div>
          </template>
        </div>
      </el-scrollbar>
      ...
      <div>
        <el-button color="#16D0FF" style="height: 40px; width: 128px">
          <ChatDotRound
            ...
            "
          />
          <span style="font-size: 18px; color: rgba(255, 255, 255, 1)">常用语</span>
        </el-button>
        <el-input
          @change="sendChatMsg"
          v-model="msgInfo"
...
const chatMessages = reactive({
  roomName: 'A2-223',
  meetingId: '8803061667463168',
  roomId: '8297097464512512',
...
const openMeetingChat = () => {
  // 更新数据表的会议
  axios
    .post('/api/call-service-base/updateMeetingInfo', {})
    .then((res) => res.json)
    .then((r) => {
      console.log(r)
    })
